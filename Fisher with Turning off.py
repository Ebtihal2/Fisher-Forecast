...
class FisherCalculator:
    def __init__(self, Gmu, CLTT_String_File, CLTE_String_File,CLBB_String_File,CLEE_String_File,TT,TE,BB,EE):
      ...
      #Calculating A in terms of Gmu
            ...
            self.A = (self.Gmu)**2 * SigmaTot_tt2 *(1.27*10**(-6))**(-2) *(SigmaStr_tt2)**(-1)
    ...
    def Gmu_variation(self,TT,TE,EE,BB):
        ...
        for i in range(2,len(self.yt)):
            SigmaTot_tt2 += ((2*i+1)/(4*np.pi))* ((cl_t[i] *(2*np.pi))/(i*(i+1)))
            SigmaStr_tt2 += ((2*i+1)/(4*np.pi))* ( (self.yt[i]+ self.zt[i]+self.tt[i])*((2*np.pi))/(i*(i+1)))
        A1 = (self.Gmu*1.01)**2 * SigmaTot_tt2 *(1.27*10**(-6))**(-2) *(SigmaStr_tt2)**(-1)
        for i in self.ls[ (self.ls < len(cl_t))& (self.ls <len(self.yt))& (self.ls > 1)]:
            Cl_11 = A1*(self.yt[i]+ self.zt[i]+self.tt[i])*(2*np.pi)/(i*(i+1)) + ((cl_t[i]*(2*np.pi))/(i*(i+1)))
            Cl_12 = A1*(self.yte[i]+ self.zte[i]+self.te[i])*(2*np.pi)/(i*(i+1))+((cl_te[i]*(2*np.pi))/(i*(i+1)))
            Cl_22 = A1*(self.ye[i]+ self.ze[i]+self.te[i])*(2*np.pi)/(i*(i+1)) + ((cl_e[i]*(2*np.pi))/(i*(i+1)))
            Cl_33 = A1*(self.yb[i]+ self.zb[i])*(2*np.pi)/(i*(i+1)) + ((cl_b[i]*(2*np.pi))/(i*(i+1))) 
            if TT == True:
                Cl_11 = self.A*(self.yt[i]+ self.zt[i]+self.tt[i])*(2*np.pi)/(i*(i+1)) + ((cl_t[i]*(2*np.pi))/(i*(i+1)))
            if TE == True: 
                Cl_12 = self.A*(self.yte[i]+ self.zte[i]+self.te[i])*(2*np.pi)/(i*(i+1))+((cl_te[i]*(2*np.pi))/(i*(i+1)))
    ...
    #Using SPT 2020 data, noiseEEE = noiseBBB is a relatively good approximation. 
        ...
        for row in range(7):
            for column in range(7):
                for i in range(100,2996):
                        Clinv = np.linalg.inv((self.Parameters_CLmatrix[variables[7]])[i])
                        self.dCldtheta_row = ((((self.Parameters_CLmatrix[variables[row]])[i]) - ((self.Parameters_CLmatrix[variables[7]])[i])) / ((0.01)*cl_dict_values[variables[row]]))
                        self.dCldtheta_column = ((((self.Parameters_CLmatrix[variables[column]])[i]) - ((self.Parameters_CLmatrix[variables[7]])[i])) / ((0.01)*cl_dict_values[variables[column]]))
...
fisher1= FisherCalculator(10**(-7),'cl_tt120.d','cl_te120.d','cl_bb120.d','cl_ee120.d',True,False, False, False)
fisher1.Gmu_variation(True,False,False,False)
fisher1.Fisher_matrix(1, dd['cl_residual']['TT'],dd['cl_residual']['EE'],dd['cl_residual']['EE'],1500 / 41253,1)
...
#without TT
fishertt= FisherCalculator(10**(-7),'cl_tt120.d','cl_te120.d','cl_bb120.d','cl_ee120.d',False,False, False, False)
fishertt.Gmu_variation(False,False,False,False)
fishertt.Fisher_matrix(1, dd['cl_residual']['TT'],dd['cl_residual']['EE'],dd['cl_residual']['EE'],1500 / 41253,1)
...
#without BB
fisherbb= FisherCalculator(10**(-7),'cl_tt120.d','cl_te120.d','cl_bb120.d','cl_ee120.d',False,False,False,True)
fisherbb.Gmu_variation(False,False,False,True)
fisherbb.Fisher_matrix(1, dd['cl_residual']['TT'],dd['cl_residual']['EE'],dd['cl_residual']['EE'],1500 / 41253,1)
